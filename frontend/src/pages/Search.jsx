...
const Search = () => {
    ...
    const handleChange =(e)=>{
        ...
        if (e.target.id === 'parking' || e.target.id === 'furnished' || e.target.id === 'offer') {
            setSidebardata({...sidebardata,[e.target.id] : e.target.checked || e.target.checked === 'true'? true : false}) //here we're checking datatype of (checked) that means if e.target.checked (boolean) or e.target.checked === 'true'(string) whatever, the value of e.target.id is true         
        }
        if (e.target.id === 'sort_order') {
            const sort  = e.target.value.split('_')[0] || 'created_at';//example: if e.target.value is "regularPrice_desc", it would return ["regularPrice", "desc"].thus sort contain (regularPrice) and order contain (desc)
    ...
    const onShowMoreClick =async()=>{
        ...
        const urlParams = new URLSearchParams(location.search);// get data through url
